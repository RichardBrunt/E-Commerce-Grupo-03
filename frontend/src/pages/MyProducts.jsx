...
import { createProduct, deleteProduct, listProducts, updateProduct, listCategories, createCategory, createModel } from '@/services/api.js'
...
export default function MyProducts(){
  ...
  const [form, setForm] = useState({ name:'', price:0, stock:0, image:'', imagesText:'', description:'', categoryId:'', modelId:'' })
  const [categories, setCategories] = useState([])
  const { data: models, refresh: refreshModels } = useModelsByCategory(form.categoryId)
  ...
  const resetForm = () => { setForm({ name:'', price:0, stock:0, image:'', imagesText:'', description:'', categoryId:'', modelId:'' }); setNewModel('') }
  ...
  const goEdit = (p) => { 
    ...
    setForm({ 
      ...
      modelId: mid !== undefined && mid !== null ? String(mid) : ''
    }) 
  }
  ...
  const addModel = async () => {
    ...
    if (!name || !cid) return
    try {
      const created = await createModel(cid, { name })
      // recargar modelos para la categoría actual
      await refreshModels()
      setForm(prev => ({ ...prev, modelId: String(created.id) }))
      setNewModel('')
    } catch (e) {
      setError('No se pudo crear el modelo')
    }
  }
          ...
          <form onSubmit={onSubmit} className="checkout-form">
            ...
            <div className="form-row">
              <div className="form-group">
                ...
                <select value={form.categoryId} onChange={e=>setForm({...form, categoryId: e.target.value, modelId: ''})}>
            ...
            <div className="form-row">
              <div className="form-group">
                ...
                <select value={form.modelId} onChange={e=>setForm({...form, modelId: e.target.value})} disabled={!form.categoryId}>
                  <option value="">Seleccione modelo</option>
                  {models.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}
                </select>
                {!form.categoryId && <small className="checkout-subtext">Seleccione una categoría para listar sus modelos</small>}
              </div>
              <div className="form-group">
                <label>Nuevo modelo</label>
                <div style={{ display:'grid', gridTemplateColumns:'1fr auto', gap: '.5rem' }}>
                  <input placeholder="Nombre de modelo" value={newModel} onChange={e=>setNewModel(e.target.value)} disabled={!form.categoryId} />
                  <button type="button" className="btn-primary" onClick={addModel} disabled={!form.categoryId}>Agregar</button>
                </div>
                {!form.categoryId && <small className="checkout-subtext">Seleccione una categoría antes de crear un modelo</small>}
              </div>
            </div>
